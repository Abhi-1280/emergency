...
class Vehicle(pygame.sprite.Sprite):
    ...
    def check_safe_distance(self, new_x, new_y):
        for vehicle in simulation:
            if vehicle != self and vehicle.direction == self.direction and vehicle.lane == self.lane:
                if self.direction in ['right', 'left']:
                    if abs(new_x - vehicle.x) < safe_distance:
                        if not self.isEmergency:
                            self.speed = max(0, vehicle.speed)
                        return False
                else:
                    if abs(new_y - vehicle.y) < safe_distance:
                        if not self.isEmergency:
                            self.speed = max(0, vehicle.speed)
                        return False
        return True
    ...
    def update_speed(self):
        ...
        if current_time - self.last_speed_update > 0.5:
            if self.isEmergency:
                ...
                self.speed = min(self.initial_speed * 1.2, self.speed + 0.1)
            elif self.moving:
                self.speed = min(self.initial_speed, self.speed + 0.05)
    ...
    while True:
        ...
        if len(vehicles[directionNumbers[direction_number]][lane_number]) > 0:
            ...
            if (directionNumbers[direction_number] in ['right', 'down'] and 
                abs(last_vehicle.x - x[directionNumbers[direction_number]][lane_number]) > safe_distance * 2):
