...
class Vehicle(pygame.sprite.Sprite):
    ...
    def check_emergency_behind(self):
        ...
        for vehicle in vehicles[self.direction][self.lane]:
            if vehicle.isEmergency and not vehicle.crossed:
                if self.direction == 'right' and vehicle.x < self.x and abs(vehicle.x - self.x) < emergency_gap * 2:
                    return True
                elif self.direction == 'left' and vehicle.x > self.x and abs(vehicle.x - self.x) < emergency_gap * 2:
                    return True
                elif self.direction == 'down' and vehicle.y < self.y and abs(vehicle.y - self.y) < emergency_gap * 2:
                    return True
                elif self.direction == 'up' and vehicle.y > self.y and abs(vehicle.y - self.y) < emergency_gap * 2:
                    return True
        return False

    def find_best_lane(self):
        available_lanes = [i for i in range(3) if i != self.lane]
        best_lane = None
        max_safety = 0
        
        for new_lane in available_lanes:
            safety_score = self.check_lane_safety(new_lane)
            if safety_score > max_safety:
                max_safety = safety_score
                best_lane = new_lane
        
        return best_lane if max_safety > 0 else None

    def check_lane_safety(self, target_lane):
        safety_score = 100
        for vehicle in vehicles[self.direction][target_lane]:
            if self.direction in ['right', 'left']:
                distance = abs(vehicle.x - self.x)
            else:
                distance = abs(vehicle.y - self.y)
            if distance < safe_distance:
                safety_score -= (safe_distance - distance)
        return max(0, safety_score)

    def give_way_to_emergency(self):
    ...
    def update_lane_change(self):
        ...
        if self.direction in ['right', 'left']:
            target_y = y[self.direction][self.target_lane]
            if abs(self.y - target_y) > lane_change_speed:
                self.y += lane_change_speed if target_y > self.y else -lane_change_speed
            else:
                self.complete_lane_change()
        else:
            target_x = x[self.direction][self.target_lane]
            if abs(self.x - target_x) > lane_change_speed:
                self.x += lane_change_speed if target_x > self.x else -lane_change_speed
            else:
                self.complete_lane_change()
    ...
    def complete_lane_change(self):
        ...
        vehicles[self.direction][self.target_lane].append(self)
        self.changing_lane = False
        self.emergency_slowdown = False
        self.speed = self.initial_speed
...
def move_vehicle(vehicle):
    if vehicle.check_emergency_behind():
        if not vehicle.changing_lane and not vehicle.emergency_slowdown:
            vehicle.give_way_to_emergency()
    
    if vehicle.changing_lane:
        vehicle.update_lane_change()
        return

    if vehicle.emergency_slowdown:
        if not vehicle.check_emergency_behind():
            vehicle.emergency_slowdown = False
            vehicle.speed = vehicle.initial_speed
    ...
    if not vehicle.moving and not vehicle.changing_lane:
    ...
    while True:
        ...
        if len(vehicles[directionNumbers[direction_number]][lane_number]) > 0:
            ...
            if (directionNumbers[direction_number] in ['right', 'down'] and 
                abs(last_vehicle.x - x[directionNumbers[direction_number]][lane_number]) > min_gap * 1.5):
    ...
    def __init__(self):
        ...
        while True:
            ...
            for vehicle in simulation:
                if vehicle.check_emergency_behind() and not vehicle.changing_lane:
