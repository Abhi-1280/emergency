...
class EmergencyQueue:
    ...
    def remove(self, vehicle):
        ...
        if vehicle == self.current:
            self.current = None
            self.process_next()
    
    def process_next(self):
        if not self.queue or self.current:
            return
        self.current = self.queue[0]
        self.processing = True
    
    def is_current(self, vehicle):
        return vehicle == self.current
...
def detectEmergencyVehicles():
    ...
    return len(emergency_vehicles) > 0, emergency_vehicles
...
class Vehicle(pygame.sprite.Sprite):
    ...
    def can_move(self):
        if self.crossed == 1:
            return True
        if self.isEmergency:
            return emergency_queue.is_current(self)
        if not emergency_queue.current:
            return True
        if emergency_queue.current.direction_number == self.direction_number:
            return True
        return False
    def move(self):
        ...
        if self.direction == 'right':
            if self.crossed == 0 and self.x + self.currentImage.get_rect().width > stopLines[self.direction]:
                self.crossed = 1
                if self.isEmergency:
                    emergency_queue.remove(self)
                vehicles[self.direction]['crossed'] += 1
            if self.willTurn == 1:
                if self.crossed == 0 or self.x + self.currentImage.get_rect().width < mid[self.direction]['x']:
                    if (self.x + self.currentImage.get_rect().width <= self.stop or (currentGreen == 0 and currentYellow == 0) or self.crossed == 1 or emergency_queue.is_current(self)) and (self.index == 0 or self.x + self.currentImage.get_rect().width < (vehicles[self.direction][self.lane][self.index-1].x - gap2)):
                        self.x += self.speed
                else:
                    if self.turned == 0:
                ...
                if (self.x + self.currentImage.get_rect().width <= self.stop or self.crossed == 1 or (currentGreen == 0 and currentYellow == 0) or emergency_queue.is_current(self)) and (self.index == 0 or self.x + self.currentImage.get_rect().width < (vehicles[self.direction][self.lane][self.index-1].x - gap2)):
                    self.x += self.speed
            ...
            if self.willTurn == 1:
                if self.crossed == 0 or self.y + self.currentImage.get_rect().height < mid[self.direction]['y']:
                    if (self.y + self.currentImage.get_rect().height <= self.stop or (currentGreen == 1 and currentYellow == 0) or self.crossed == 1 or emergency_queue.is_current(self)) and (self.index == 0 or self.y + self.currentImage.get_rect().height < (vehicles[self.direction][self.lane][self.index-1].y - gap2)):
                        self.y += self.speed
                else:
                    if self.turned == 0:
                        self.rotateAngle += rotationAngle
                ...
                if (self.y + self.currentImage.get_rect().height <= self.stop or self.crossed == 1 or (currentGreen == 1 and currentYellow == 0) or emergency_queue.is_current(self)) and (self.index == 0 or self.y + self.currentImage.get_rect().height < (vehicles[self.direction][self.lane][self.index-1].y - gap2)):
                    self.y += self.speed
            ...
            if self.willTurn == 1:
                if self.crossed == 0 or self.x > mid[self.direction]['x']:
                    if (self.x >= self.stop or (currentGreen == 2 and currentYellow == 0) or self.crossed == 1 or emergency_queue.is_current(self)) and (self.index == 0 or self.x > (vehicles[self.direction][self.lane][self.index-1].x + vehicles[self.direction][self.lane][self.index-1].currentImage.get_rect().width + gap2)):
                        self.x -= self.speed
                else:
                    if self.turned == 0:
                        self.rotateAngle += rotationAngle
                ...
                if (self.x >= self.stop or self.crossed == 1 or (currentGreen == 2 and currentYellow == 0) or emergency_queue.is_current(self)) and (self.index == 0 or self.x > (vehicles[self.direction][self.lane][self.index-1].x + vehicles[self.direction][self.lane][self.index-1].currentImage.get_rect().width + gap2)):
                    self.x -= self.speed
            ...
            if self.willTurn == 1:
                if self.crossed == 0 or self.y > mid[self.direction]['y']:
                    if (self.y >= self.stop or (currentGreen == 3 and currentYellow == 0) or self.crossed == 1 or emergency_queue.is_current(self)) and (self.index == 0 or self.y > (vehicles[self.direction][self.lane][self.index-1].y + vehicles[self.direction][self.lane][self.index-1].currentImage.get_rect().height + gap2)):
                        self.y -= self.speed
                else:
                    if self.turned == 0:
                        self.rotateAngle += rotationAngle
                ...
                if (self.y >= self.stop or self.crossed == 1 or (currentGreen == 3 and currentYellow == 0) or emergency_queue.is_current(self)) and (self.index == 0 or self.y > (vehicles[self.direction][self.lane][self.index-1].y + vehicles[self.direction][self.lane][self.index-1].currentImage.get_rect().height + gap2)):
                    self.y -= self.speed
def initialize():
...
def handleEmergencyVehicle():
    ...
    if emergency_queue.current:
        if currentGreen != emergency_queue.current.direction_number:
            currentGreen = emergency_queue.current.direction_number
            nextGreen = (currentGreen + 1) % noOfSignals
            currentYellow = 0
            for i in range(noOfSignals):
                if i == currentGreen:
...
class Main:
    def __init__(self):
        ...
        while True:
            ...
            if emergency_queue.current:
                emergencyText = self.font.render(f"EMERGENCY VEHICLE IN LANE {emergency_queue.current.direction_number + 1}", True, (255,0,0), (255,255,255))
                self.screen.blit(emergencyText, (1100,100))
                if len(emergency_queue.queue) > 1:
                    waitingText = self.font.render(f"EMERGENCY VEHICLES WAITING: {len(emergency_queue.queue) - 1}", True, (255,0,0), (255,255,255))
                    self.screen.blit(waitingText, (1100,130))
