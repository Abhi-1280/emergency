...
class IntersectionManager:
    def __init__(self):
        self.crossing_vehicles = set()
        self.waiting_time = {}
        self.safe_distance = 50
        
    def can_cross(self, vehicle):
        if vehicle.isEmergency:
            return True
            
        for crossing_vehicle in self.crossing_vehicles:
            if self.check_collision_risk(vehicle, crossing_vehicle):
                return False
        return True
    ...
    def register_vehicle(self, vehicle):
        self.crossing_vehicles.add(vehicle)
        self.waiting_time[vehicle] = time.time()
    
    def unregister_vehicle(self, vehicle):
        if vehicle in self.crossing_vehicles:
            self.crossing_vehicles.remove(vehicle)
            self.waiting_time.pop(vehicle, None)
...
def detectEmergencyVehicles():
    ...
    return len(emergency_directions) > 0, emergency_directions, emergency_vehicles
    ...
    def check_safe_distance(self):
        if self.index > 0:
            ...
            if self.direction in ['right', 'left']:
                distance = abs(self.x - lead_vehicle.x)
            else:
                distance = abs(self.y - lead_vehicle.y)
            return distance >= safe_distance
        return True

    def is_in_intersection(self):
        intersection_x_range = (590, 800)
        intersection_y_range = (330, 535)
        return (intersection_x_range[0] <= self.x <= intersection_x_range[1] and 
                intersection_y_range[0] <= self.y <= intersection_y_range[1])

    def can_move(self):
        if self.isEmergency:
            return self.check_safe_distance()
            
        if self.waiting:
            if time.time() - self.wait_start_time >= self.max_wait_time:
                self.waiting = False
                return True
            return False
            
        if self.is_in_intersection() and not intersection_manager.can_cross(self):
            if not self.waiting:
                self.waiting = True
                self.wait_start_time = time.time()
            return False
            
        emergency_present, emergency_directions, _ = detectEmergencyVehicles()
        ...
        if self.direction == 'right':
            if self.crossed == 0 and self.x + self.currentImage.get_rect().width > stopLines[self.direction]:
                ...
                if self.is_in_intersection():
                    intersection_manager.register_vehicle(self)
            ...
            if self.willTurn == 1:
                if self.crossed == 0 or self.x + self.currentImage.get_rect().width < mid[self.direction]['x']:
                    ...
                    if self.turned == 0:
                        ...
                        if self.rotateAngle == 90:
                            self.turned = 1
                            intersection_manager.unregister_vehicle(self)
                    else:
                ...
                if (self.x + self.currentImage.get_rect().width <= self.stop or self.crossed == 1 or (currentGreen == 0 and currentYellow == 0) or self.isEmergency) and (self.index == 0 or self.x + self.currentImage.get_rect().width < (vehicles[self.direction][self.lane][self.index-1].x - gap2)):
                    self.x += self.speed
                    if not self.is_in_intersection() and self.crossed == 1:
                        intersection_manager.unregister_vehicle(self)
...
def handleEmergencyVehicle():
    ...
    emergency_present, emergency_directions, emergency_vehicles = detectEmergencyVehicles()
    if emergency_present:
    ...
    def __init__(self):
        ...
        while True:
            ...
            for vehicle in simulation:
                ...
                if (vehicle.x < -100 or vehicle.x > 1500 or 
                    vehicle.y < -100 or vehicle.y > 900):
            ...
            for vehicle in vehicles_to_remove:
                ...
                if vehicle in intersection_manager.crossing_vehicles:
                    intersection_manager.unregister_vehicle(vehicle)
                direction = vehicle.direction
                lane = vehicle.lane
                if vehicle in vehicles[direction][lane]:
                    vehicles[direction][lane].remove(vehicle)
